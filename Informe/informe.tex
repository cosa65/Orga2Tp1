...
\begin{itemize}
\item Levantar dos p\'{i}xeles en un XMM pasando R, G, B y A a Words.\\
\item Luego en otros dos XMM hacer rotaciones para que queden alineados verticalmente los R, G y B de ambos p\'{i}xeles. \\
\item Con una suma vertical (paddw) obtenemos un XMM con las sumas R+G+B de ambos p\'{i}xeles. Luego con dos pshufw hacemos 'broadcast' de la suma en sus respectivas partes. (Este proceso lo hacemos dos veces por ciclo, para trabajar con 4 p\'{i}xeles por iteraci\'{o}n).\newpage

\begin{table}[h]
\center
\begin{tabular}{|l|l|l|l|l|l|l|l|}
\hline
$R_1$    & $G_1$    & $B_1$    & $A_1$   & $R_2$    & $G_2$    & $B_2$    & $A_2$ \\ \hline
$G_1$    & $B_1$    & $A_1$    & $R_2$    & $G_2$    & $B_2$    & $A_2$    & 0  \\ \hline
$B_1$    & $A_1$    & $R_2$    & $G_2$    & $B_2$    & $A_2$    & 0     & 0  \\ \hline
$SUMA_1$ & X     & X     & X     & $SUMA_2$ & X     & X     & X  \\ \hline
$SUMA_1$ & $SUMA_1$ & $SUMA_1$ & $SUMA_1$ & $SUMA_2$ & $SUMA_2$ & $SUMA_2$ & $SUMA_2$   \\ \hline
\end{tabular}
\end{table} 

\item Luego, en lugar de hacer el branching innecesario, utilizamos 'pcmpgtw' para compararlos con 4 m\'{a}scaras que contengan 96, 288, 480 y 672 en Words. De esta forma, obtenemos m\'{a}scaras que contienen unos en aquellos words que son mayores, y ceros donde son menores o iguales. \\
\item Aplicando restas, pasamos de tener dos m\'{a}scaras que nos indican (x < 96) y (x < 288) a tener una que nos indica (96 <= x < 288) (Por dar un ejemplo). \\
\item A estas m\'{a}scaras les aplicamos la instrucci\'{o}n pandw junto con una m\'{a}scara con el resultado correspondiente (en este caso 64) para obtener ese resultado en los words donde hab\'{i}an unos, y ceros en caso contrario. Sumando todas estas m\'{a}scaras finales obtenemos el resultado final, en words. \\
\item Con un simple 'packusdw' transformamos estos dos XMM con los resultados finales en words, en uno solo que tiene los 4 p\'{i}xeles a guardar, en Bytes. \\
